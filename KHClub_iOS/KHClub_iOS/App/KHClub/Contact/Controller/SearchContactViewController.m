...
- (void)viewDidLoad {
    ...
    [self setNavBarTitle:KHClubString(@"添加通讯录好友")];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    contactLabel.text                 = [NSString stringWithFormat:@"%@", self.contactDic[contact.phone]];;
    ...
    //如果已经添加了
    if (contact.is_friend == 1) {
        [addBtn setBackgroundColor:[UIColor colorWithHexString:ColorGary]];
        [addBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [addBtn setTitle:KHClubString(@"Contact_SearchContactAdded") forState:UIControlStateNormal];
        addBtn.enabled = NO;
    }else{
        addBtn.enabled = YES;
        [addBtn setBackgroundColor:[UIColor colorWithHexString:ColorGold]];
        [addBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [addBtn setTitle:KHClubString(@"Contact_SearchContactAdd") forState:UIControlStateNormal];
    }
...
- (void)addFriendCommitWith:(ContactModel *)contactModel
{
    ...
    NSString *buddyName = [ToolsManager getCommonTargetId:contactModel.uid];
