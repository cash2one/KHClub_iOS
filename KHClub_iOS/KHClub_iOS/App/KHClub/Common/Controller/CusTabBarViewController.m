...
@interface CusTabBarViewController ()<IChatManagerDelegate, EMCallManagerDelegate>
...
//    _backView.image                  = [UIImage imageNamed:@"tabbar"];
    _backView.backgroundColor        = [UIColor colorWithHexString:ColorBlack];
    _backView.userInteractionEnabled = YES;
    [self.tabBar addSubview:_backView];

    _coverView                       = [[UIView alloc] init];
    _coverView.backgroundColor       = [UIColor colorWithHexString:ColorGold];
...
- (void)createVC
{
    ...
    for (NSDictionary * dic in arr) {
        ...
        bVC.hideLeftBtn          = YES;
        NSString * title         = KHClubString([dic objectForKey:@"title"]);
        [_titleArr addObject:title];
        [_vcArr addObject:bVC];
        [_normaleArr addObject:dic[@"normalPic"]];
        [_selectedArr addObject:dic[@"selectedPic"]];
        
        if ([bVC isKindOfClass:[ChatListViewController class]]) {
            _chatListVC = (ChatListViewController *)bVC;
        }else if ([bVC isKindOfClass:[ContactsViewController class]]) {
            _contactsVC = (ContactsViewController *)bVC;
        }
    }
...
//    }
    ...
    btn.selected         = YES;
    self.coverView.frame = btn.frame;
...
- (void)registerNotification
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(badgeNotify:) name:@"setupUntreatedApplyCount" object:nil];
...
//        NSString *title = [[UserProfileManager sharedInstance] getNickNameWithUsername:message.from];
        NSString *title = [[IMUtils shareInstance] getNickName:message.from];
