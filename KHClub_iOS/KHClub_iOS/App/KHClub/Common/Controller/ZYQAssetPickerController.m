...
- (void)setupViews
{
    ...
    titleLabel.text = KHClubString(@"Common_Gallery");
    titleLabel.font = [UIFont systemFontOfSize:18];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = [UIColor colorWithHexString:ColorWhite];
...
- (void)setupButtons
{
    ...
    [[UIBarButtonItem alloc] initWithTitle:StringCommonFinish
                                     style:UIBarButtonItemStylePlain
                                    target:self
                                    action:@selector(finishPickingAssets:)];

    self.navigationController.navigationBar.tintColor = [UIColor colorWithHexString:ColorWhite];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld %@", nil), (long)_numberOfPhotos, KHClubString(@"News_Publish_Pictures")];
...
- (NSString *)accessibilityLabel
{
    ...
    return [label stringByAppendingFormat:NSLocalizedString(@"%ld %@", nil), (long)[self.assetsGroup numberOfAssets], KHClubString(@"News_Publish_Pictures")];
}
...
- (void)setupButtons
{
    ...
    {
        self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:StringCommonCancel
                                         style:UIBarButtonItemStylePlain
                                        target:self
...
//    title.text              = NSLocalizedString(@"此应用无法使用您的照片或视频。", nil);
    title.text              = KHClubString(@"News_Publish_NoAllowPictures");
...
//    message.text            = NSLocalizedString(@"你可以在「隐私设置」中启用存取。", nil);
    title.text              = KHClubString(@"News_Publish_NoAllowPictures2");    
...
//    title.text              = NSLocalizedString(@"没有照片或视频。", nil);
    title.text              = KHClubString(@"News_Publish_NoPictures");
