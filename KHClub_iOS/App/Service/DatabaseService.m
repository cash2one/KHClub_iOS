...
-(void)copyFileDatabase
{
    ...
    _dbPath = [documentsDirectory stringByAppendingPathComponent:@"khclub.db"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:_dbPath]) {
        debugLog(@"Database File Exsit");
    }else {
        NSString *resourceFolderPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"khclub.db"];
